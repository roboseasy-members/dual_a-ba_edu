...
def make_robot_from_config(config: RobotConfig) -> Robot:
    ...
    elif config.type == "bi_so102_follower":
        from .bi_so102_follower import BiSO102Follower

        return BiSO102Follower(config)
    elif config.type == "bi_so102_client":
        from .bi_so102_follower import BiSO102Client

        return BiSO102Client(config)
    else:
        try:
